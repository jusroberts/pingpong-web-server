...
class StatsManager
  ...
  def get_player_stats(player_id, player_count)
    ...
    WeeklyStats.where(player_id: player_id, player_count: player_count).all.each do |weekly_stat|
      player_stats.process_stat_entry!(weekly_stat, 7)
    end

    # Pull daily stats in progress
    daily_stats = DailyStats.where(player_id: player_id).last

    # Aggregate into running total
    player_stats.process_stat_entry!(daily_stats, 1)
    player_stats
  end
end